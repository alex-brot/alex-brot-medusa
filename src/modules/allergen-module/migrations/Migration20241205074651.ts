...
export class Migration20241205074651 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table if not exists "allergen" ("id" text not null, "name" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "allergen_pkey" primary key ("id"));');
    this.addSql('CREATE INDEX IF NOT EXISTS "IDX_allergen_deleted_at" ON "allergen" (deleted_at) WHERE deleted_at IS NULL;');
  ...
  async down(): Promise<void> {
    this.addSql('alter table if exists "allergen_product" drop constraint if exists "allergen_product_allergen_id_foreign";');

    this.addSql('alter table if exists "allergen_product" drop constraint if exists "allergen_product_product_id_foreign";');

    this.addSql('drop table if exists "allergen" cascade;');
